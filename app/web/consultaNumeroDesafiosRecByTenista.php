...
if (isset($_POST['idTenistas']) && isset($_POST['idCategoria']))
{
	...
	$sql = "select COUNT(*) from Desafio where TenistaDesafiado='$idTenistas' AND Jogado=1 AND Categoria_idCategoria=$idCategoria AND YEAR(DATA)=YEAR(CURRENT_DATE) AND MONTH(DATA)=MONTH(CURRENT_DATE)";
...
if (mysqli_num_rows($res) > 0)
{
	...
	$qtdDesafios = $row["COUNT(*)"];
	
	
	//echo "Qtd Desafios: $qtdDesafiosFeitos\n\n";

	$result["result"]=$qtdDesafios; 
